...
- [ ] Task 1: Create `DevServer` orchestrator class (AC: #1, #2, #4, #5, #7)
  - [ ] 1.1 Create `jsf-autoreload-core/src/main/java/it/bstz/jsfautoreload/DevServer.java`
  - [ ] 1.2 Constructor takes `DevServerConfig`
  - [ ] 1.3 `start()` method: validate config, initialize components, enter blocking dev loop
  - [ ] 1.4 Validate output directory exists at start — throw `JsfAutoreloadConfigException` if not
  - [ ] 1.5 Initialize in order: FileChangeWatcher -> DevWebSocketServer -> dev loop
  - [ ] 1.6 Wire file change callback: on change -> copy file to outputDir (preserving relative path) -> broadcast reload
  - [ ] 1.7 Wire file delete callback: on delete -> skip copy -> broadcast reload
  - [ ] 1.8 Log startup confirmation message with port and watched directories
- [ ] Task 2: Implement debounce coalescing (AC: #3)
  - [ ] 2.1 Use `ScheduledExecutorService` from `DevServerConfig` (or create default if not provided)
  - [ ] 2.2 On file event: schedule reload broadcast after debounce window (default 300ms)
  - [ ] 2.3 If another event arrives within the window: cancel pending, reschedule
  - [ ] 2.4 Result: rapid changes coalesce into a single reload
- [ ] Task 3: Implement graceful shutdown (AC: #6, #7)
  - [ ] 3.1 Implement `Closeable` interface
  - [ ] 3.2 `close()` shuts down in reverse order: cancel debounce executor -> close WebSocket server -> close file watcher
  - [ ] 3.3 Each component gets 1-second timeout via `executor.awaitTermination(1, SECONDS)`
  - [ ] 3.4 Total shutdown within 2 seconds (NFR16)
  - [ ] 3.5 Register shutdown hook for Ctrl+C handling
- [ ] Task 4: Implement file copy logic (AC: #1, #2)
  - [ ] 4.1 Copy changed file to outputDir preserving relative path from watch directory
  - [ ] 4.2 Use `java.nio.file.Files.copy()` with `REPLACE_EXISTING`
  - [ ] 4.3 Create parent directories if needed (`Files.createDirectories()`)
  - [ ] 4.4 On delete: skip copy, still trigger reload
  - [ ] 4.5 Log file operations at FINE level
- [ ] Task 5: Write comprehensive tests (AC: #1-#7)
  - [ ] 5.1 Create `jsf-autoreload-core/src/test/java/it/bstz/jsfautoreload/DevServerTest.java`
  - [ ] 5.2 Test file modify -> copy + broadcast
  - [ ] 5.3 Test file delete -> no copy + broadcast
  - [ ] 5.4 Test debounce: rapid changes -> single reload (inject controllable executor)
  - [ ] 5.5 Test startup with missing output dir -> exception
  - [ ] 5.6 Test graceful shutdown releases all resources
  - [ ] 5.7 Test try-with-resources pattern
  - [ ] 5.8 Use `@TempDir` for file system, `CountDownLatch` for async assertions
...
- [Source: jsf-autoreload-plugin/src/main/java/it/bstz/jsfautoreload/JsfDevTask.java — current 293-line orchestration to extract from]
- [Source: _bmad-output/planning-artifacts/prd.md#NFR1 — sub-2s feedback loop]
- [Source: _bmad-output/planning-artifacts/prd.md#NFR6 — 300ms debounce]
- [Source: _bmad-output/planning-artifacts/prd.md#NFR16 — 2s graceful shutdown]
